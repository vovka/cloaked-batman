  ...
  var drawingManager = new google.maps.drawing.DrawingManager({
    ...
    google.maps.event.addDomListener(document.getElementById('saveShape'), 'click', function(evt){
      ...
      if(selectedShape && selectedShape.type != google.maps.drawing.OverlayType.MARKER){
        ...
        switch(selectedShape.type){
          case "polygon":
            coord = selectedShape.getPath().getArray();
            break;
          case "rectangle":
            coord = selectedShape.getBounds();
            break;
        }
        var shapeToObj = new ShapeToObj();
        shapeToObj.shapeType = selectedShape.type;
        shapeToObj.shapeCoord = coord;
        google.maps.event.addDomListener(document.getElementById('sendShape'), 'click', function(){
          $.ajax({
            url: "/map/save",
            type: "POST",
            data: { admin: 1 },
          ...
          // var shapeJson = JSON.stringify(shapeToObj);
          // var req = new XMLHttpRequest();
          // req.onreadystatechange = function(){
            // if (req.readyState != 4) return;
            // console.info("Shape is saved");
            // console.log(req.responseText);
          // }
          // req.open("POST", "/map/save", true);
        ...
        evt.target.setAttribute("data-toggle", "");
      }
    });
